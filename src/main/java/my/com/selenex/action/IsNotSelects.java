 ...
 *
 */
public class IsNotSelects {
	
	static Logger logger = Logger.getLogger(IsNotSelects.class);
	...
	private ResultReport execute (
		...
		resultReport.setExpected("To ensure NONE of items ("+ scenario.getValue().replaceAll("\n", ",")+") been selected, ");
		...
		try {
			...
			if (!select.isMultiple()) {
				resultReport.setActual("Action '"+ scenario.getAction() + "' NOT applicable for single select. use action 'is-not-select' instead");
