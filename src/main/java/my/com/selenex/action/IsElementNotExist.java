...
public class IsElementNotExist {
	
	static Logger logger = Logger.getLogger(IsElementNotExist.class);
	...
	private ResultReport execute (
		...
		resultReport.setExpected("Validate isNotExist  (" + scenario.getSelectorType() + ") :" + scenario.getSelectorString());
		try {
			...
			if (scenario.getSelectorType().equalsIgnoreCase("xpath")) {
				...
				webElement = helper.getDriver().findElement(By.cssSelector(scenario.getSelectorType()));
			}
